...
public class AppJmsSpout extends BaseRichSpout implements MessageListener {
    private static final Logger LOG = LoggerFactory.getLogger(AppJmsSpout.class);
    ...
    private class RecoveryTask extends TimerTask {
        ...
        public void run() {
            synchronized (AppJmsSpout.this.recoveryMutex) {
                if (AppJmsSpout.this.hasFailures()) {
                    try {
                        LOG.info("Recovering from a message failure.");
                        AppJmsSpout.this.getSession().recover();
                        AppJmsSpout.this.recovered();
                    } catch (JMSException e) {
